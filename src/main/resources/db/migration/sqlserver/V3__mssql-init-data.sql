-- Can't use SQL Server variables functionality
-- Since there's no batching when Spring execute this script
-- It's like the GO signal get run automatically after each and every SQL command
-- Therefore any variable wouldn't exist in any subsequent command
-- primarily because those variables are only defined client-side (T-SQL feature ?)
-- This command is pointless: DECLARE @JaneId BIGINT = NEXT VALUE FOR author_seq;

-- Dan
insert into author (id, first_name, last_name, email) values (NEXT VALUE FOR author_seq, 'Dan', 'Vega', 'therealvega@gmail.com');


-- John
insert into author (id, first_name, last_name, email) values (NEXT VALUE FOR author_seq, 'John', 'Doe', 'johndoe@gmail.com');
insert into post (id, title, slug, teaser, body, active, author_id, posted_on)
        select 2, 'A Journey Through Nature', 'a-journey-through-nature',
                '<p class="blog-content">As I stepped into the wilderness, I could feel the fresh breeze caressing my face, and the soothing sounds of birds singing in the distance. The trail ahead was filled with lush greenery and colorful wildflowers. It was a perfect escape from the chaos of the city, a chance to reconnect with nature.</p>',
                '<p class="blog-content">As I stepped into the wilderness, I could feel the fresh breeze caressing my face, and the soothing sounds of birds singing in the distance. The trail ahead was filled with lush greenery and colorful wildflowers. It was a perfect escape from the chaos of the city, a chance to reconnect with nature.</p>'
                ...
                1,
                (SELECT CAST(current_value AS bigint) FROM sys.sequences WHERE name = 'author_seq'),
                '2023-03-12';
...
insert into post (id, title, slug, teaser, body, active, author_id, posted_on)
        select 4, 'Introduction to Spring Boot: Simplify Your Java Projects', 'introduction-to-spring-boot-simplify-your-java-projects',
                '<p class="blog-content">In the world of Java development, Spring Boot has become a popular framework for building robust and scalable applications. With its convention-over-configuration approach, Spring Boot simplifies the setup and configuration of Spring applications, allowing developers to focus more on writing business logic rather than dealing with boilerplate code.</p>',
                '<p class="blog-content">In the world of Java development, Spring Boot has become a popular framework for building robust and scalable applications. With its convention-over-configuration approach, Spring Boot simplifies the setup and configuration of Spring applications, allowing developers to focus more on writing business logic rather than dealing with boilerplate code.</p>'
...
-- Jane
insert into author (id, first_name, last_name, email) values (NEXT VALUE FOR author_seq, 'Jane', 'Smith', 'janesmith@gmail.com');

insert into post (id, title, slug, teaser, body, active, author_id, posted_on)
        select 3, 'Exploring the Wonders of Space', 'exploring-the-wonders-of-space',
        '<p class="blog-content">Space, the final frontier. The vastness of the universe has always fascinated humanity, igniting our curiosity and sense of wonder. As we gaze up at the night sky, we can''t help but feel a sense of awe and humility.</p>',
        '<p class="blog-content">Space, the final frontier. The vastness of the universe has always fascinated humanity, igniting our curiosity and sense of wonder. As we gaze up at the night sky, we can''t help but feel a sense of awe and humility.</p>'
...
insert into post (id, title, slug, teaser, body, active, author_id, posted_on)
        select 6, 'Good Practices for Java Developers', 'good-practices-for-java-developers',
        '<p class="blog-content">Java is a widely-used programming language known for its versatility and robustness. As a Java developer, following good practices is essential to ensure code quality, maintainability, and efficiency. Here are some key practices that every Java developer should keep in mind:</p>',
        '<p class="blog-content">Java is a widely-used programming language known for its versatility and robustness. As a Java developer, following good practices is essential to ensure code quality, maintainability, and efficiency. Here are some key practices that every Java developer should keep in mind:</p>'
